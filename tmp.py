...
while True:
    ...
    for i in range(len(HOLES)):
        NEW_HOLES.append([HOLES[i][0] - (i % 3 - 1) * 2, HOLES[i][1] - (i // 3 * 2 - 1) * 2])
    ...
    for i in order:
        if balls[i][0] != -1:
            ...
            for j in range(1, 6):
                ...
                mit = math.sqrt((whiteBall_x-tx)**2 + (whiteBall_y-ty)**2)
                dist = area / mit
                if dist >= r :
                    break
    ...
    # width, height: 목적구와 흰 공의 X좌표 간의 거리, Y좌표 간의 거리
        ...
        sub = 0
        for i in range(6):
            hx, hy = NEW_HOLES[i][0], NEW_HOLES[i][1]
    ...
    if distance > 100: power = distance * 0.4
    elif distance > 60: power = distance * 0.5
    elif distance > 30: power = distance * 0.4
    elif distance < 30: power = distance * 0.45
